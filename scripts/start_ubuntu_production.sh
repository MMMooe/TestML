...
NVIDIA_APT_MIRROR="${NVIDIA_APT_MIRROR:-https://mirrors.tuna.tsinghua.edu.cn/nvidia-cuda/ubuntu2204/x86_64}"
...
main() {
  ...
  if [[ "${SKIP_BUILD:-0}" != "1" ]]; then
    ...
    CUDA_BASE_IMAGE="$CUDA_BASE_IMAGE" NODE_BASE_IMAGE="$NODE_BASE_IMAGE" UBUNTU_APT_MIRROR="$UBUNTU_APT_MIRROR" NVIDIA_APT_MIRROR="$NVIDIA_APT_MIRROR" DOCKER_BUILDKIT="$DOCKER_BUILDKIT" COMPOSE_DOCKER_CLI_BUILD="$COMPOSE_DOCKER_CLI_BUILD" "${COMPOSE_CMD[@]}" build
  else
    log "Skipping Docker image build (SKIP_BUILD=1)"
  fi

  log "Starting production stack"
  CUDA_BASE_IMAGE="$CUDA_BASE_IMAGE" NODE_BASE_IMAGE="$NODE_BASE_IMAGE" UBUNTU_APT_MIRROR="$UBUNTU_APT_MIRROR" NVIDIA_APT_MIRROR="$NVIDIA_APT_MIRROR" "${COMPOSE_CMD[@]}" up --no-build "$@"
}
