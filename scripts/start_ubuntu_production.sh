...
TENSORRT_APT_PACKAGES="${TENSORRT_APT_PACKAGES:-python3-libnvinfer libnvinfer8 libnvinfer-plugin8 libnvinfer-bin}"
...
EOF
  ...
  if [[ "${SKIP_BUILD:-0}" != "1" ]]; then
    ...
    log "Using TensorRT APT packages: $TENSORRT_APT_PACKAGES"
    CUDA_BASE_IMAGE="$CUDA_BASE_IMAGE" NODE_BASE_IMAGE="$NODE_BASE_IMAGE" UBUNTU_APT_MIRROR="$UBUNTU_APT_MIRROR" UBUNTU_SECURITY_APT_MIRROR="$UBUNTU_SECURITY_APT_MIRROR" NVIDIA_APT_MIRROR="$NVIDIA_APT_MIRROR" APT_RETRIES="$APT_RETRIES" APT_TIMEOUT="$APT_TIMEOUT" APT_FORCE_IPV4="$APT_FORCE_IPV4" TENSORRT_APT_PACKAGES="$TENSORRT_APT_PACKAGES" DOCKER_BUILD_NETWORK="$DOCKER_BUILD_NETWORK" DOCKER_BUILDKIT="$DOCKER_BUILDKIT" COMPOSE_DOCKER_CLI_BUILD="$COMPOSE_DOCKER_CLI_BUILD" "${COMPOSE_CMD[@]}" build || fail "Docker image build failed"
